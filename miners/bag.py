...
def rec_connections(pd_df, baseucheme, positionsfeature='bag_positions',
                    ...
                    dictcheme='', print_analysis=False):
...
def rec_ecndav_ropt(positions, cheme, print_convergence=False,
                    roundpijtoecn=False):
    """Return the effective coordination number (ecn), the average bound
    distance (dav), the optimized radius (ropt), and the conective index matrix
...
def rec_ecndav(positions='bag_positions', print_convergence=False):
...
def data_from_opsbags(pd_df, new_bag_name, bags, opsbags, opsinterbags='', kind='reg'):
...
def data_from_opsdatas(pd_df, new_bag_name, new_kind, kinds, bags, opsbags,
                       opsinterbags=''):
    """It take a bag for operte over other bags.
    logicalop : np.logical_or, np.logical_and
    """
...
def class_from_svalues(pd_df, bags, bagsvals, new_class_name, operationsonbags='', addqtn=False):
    ...
    for bag in bags:
        for index in range(len(pd_df)):
            data = bag2arr(pd_df[bag][index], dtype=str)
            if len(data) != bagssize:  # problem...
                print('Different entries present different sizes...')
                breaknest = True
                break
        if breaknest:
            break
...
def classes_mixing(pd_df, classes1, classes2, classesn1='', classesn2=''):
    ...
    for index in range(len(pd_df)):
        for classa in classes1 + classes2:
            data = bag2arr(pd_df[classa][index], dtype=float)
            if len(data) != 45:  # problem...
                print(index, classa, data)
    ...
    for class1, classn1 in zip(classes1, classesn1):
        for class2, classn2 in zip(classes2, classesn2):
            ...
            new_feature_data = []
            for index in range(len(pd_df)):
                class1data = bag2arr(pd_df[class1][index], dtype=bool)
                class2data = bag2arr(pd_df[class2][index], dtype=bool)
